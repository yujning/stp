...
#include <numeric>
#include <Eigen/Dense>
#include <iostream>                                          // The printed information is used in the test and can be deleted later
...
namespace stp
{
  matrix swap_matrix(const int& m, const int& n)
  {
  ...
  matrix stp_calculation( const matrix& A, const matrix& B )
  {
    unsigned m = A.rows();
    unsigned n = A.cols();
  ...
  matrix matrix_chain_mul(const m_chain &matrix_chain)
  {
    ...
    result_matrix = stp_calculation(matrix_chain[0], matrix_chain[1]);
    for (int i = 2; i < matrix_chain.size(); i++)
      result_matrix = stp_calculation(result_matrix, matrix_chain[i]);
    ...
    while (p_variable >= 0)
    {
      ...
      if ( var != 0)  // 1:find a variable
      {
        ...
        else  // the variable appears for the not first time
        {
          ...
          {
            find_variable = true;
            matrix_chain.push_back(swap_matrix( 2, 1 << (idx[0] - idx[var]) ));
            for (int i = 1; i <= max; i++)
            {
